...
Lambda <- expand.grid(lambda1_cand,lambda2_cand) #each row is a candidate of combinations about hyperparameters
...
for (j in 1:candidate_num)
{
  lambda1 <- Lambda[j,1]
  lambda2 <- Lambda[j,2]
  output <- MTLCVX_estimator(response=response,y_list_tr,X_list_tr,R,lambda1,lambda2,rho=1,thre_ADMM=0.01,max_iter=200) #model estimation
...
output <- MTLCVX_estimator(response=response,y_list_tr,X_list_tr,R,op_lambda1,op_lambda2,rho=1,thre_ADMM=0.01,max_iter=200) #model estimation with selected parameter
